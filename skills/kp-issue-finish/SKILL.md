...
6. MUST call tool `kanpal_get_ticket` with parameter `{"key": "<ticket-key>"}`.
7. MUST call tool `kanpal_update_ticket` with parameter `{"id":"<ticket-id>"}`.

## Instructions

1. Ask user following questions to collect information:
   1. What ticket key should I finish?
   2. Should I rebase and merge KAN-10 onto develop, then clean up its worktree and branch?
   3. Should I rebase directly onto local develop, or should you provide the remote?
...
4. If there are staged changes, commit the staged changes with a Conventional Commit message whose optional scope is the ticket key:
...
7. If the rebase succeeded, merge the changes back to the base branch: `git checkout <base-branch>` then `git merge <ticket-key>`. Verify with `git log`.
8. Clean up the ticket worktree and branch (run in EVERY environment), then close the Herdr workspace if applicable:
   1. Git cleanup — unconditional, never gated on the Herdr check:
      1. Resolve the worktree path for the ticket branch: `git worktree list --porcelain` and find the `worktree <path>` block whose `branch refs/heads/<ticket-key>` line matches the ticket branch.
      ...
      5. Delete the ticket branch with `git branch -d <ticket-key>` (never `-D`). If git refuses because the branch is not fully merged, stop and ask the user. If the branch does not exist, report that it was already deleted and continue.
      6. Only touch the worktree and branch named `<ticket-key>` for this ticket; never remove worktrees or branches belonging to other tickets.
